...
import { useQuestionnaireResponsesByPatient } from '../hooks/useQuestionnaireResponsesByPatient'

interface QuestionnaireResponsesPanelProps {
  patientId: number
}

export function QuestionnaireResponsesPanel({
  patientId,
}: QuestionnaireResponsesPanelProps) {
  const { data: responses = [], isLoading, isError, error } =
    useQuestionnaireResponsesByPatient(patientId)
  ...
  return (
    <QueryBoundary isLoading={isLoading} isError={isError} error={error}>
      ...
      {responses.length === 0 && (
        <EmptyState message="Sin cuestionarios asignados." />
      )}
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Stack direction="row" spacing={1} sx={{ alignItems: 'center', flexWrap: 'wrap' }}>
                <Typography sx={{ fontSize: '14px', fontWeight: 600 }}>
                  {response.questionnaireName ?? 'Cuestionario'}
                </Typography>
      ...
      <Typography sx={{ fontSize: '11px', color: 'text.secondary' }}>
        Los cuestionarios pendientes se envían al portal del paciente; el especialista también puede
