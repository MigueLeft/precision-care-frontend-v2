...
export function MappingManagerDrawerBody({
  ...
  const createMutation = useCreateMapping(questionnaireId, { onSuccess: () => setIsCreateOpen(false) })
  const updateMutation = useUpdateMapping(questionnaireId, { onSuccess: () => setEditingId(null) })
  const deleteMutation = useDeleteMapping(questionnaireId, { onSuccess: () => setDeletingId(null) })
